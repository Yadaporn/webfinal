...
$(document).ready(() => {
    // ในที่นี้เราใช้ $(document).ready() แล้วแทน

    // คำสั่ง jQuery ด้านล่างจะรอจนกว่าหน้า HTML จะโหลดเสร็จ
    // และ DOM จะพร้อมใช้งาน
    $.ajax({
        method: 'GET',
        url: 'http://localhost/web/api/getproduct.php',
        dataType: 'json',
...
// ส่วนอื่น ๆ ของโค้ดของคุณจะไม่เปลี่ยนแปลง
// คำสั่ง JavaScript สามารถเรียกฟังก์ชัน initPage() เมื่อหน้า HTML พร้อมใช้งานแล้ว
...
function addtocart() {
    ...
    for (let i = 0; i < cart.length; i++) {
        if( productindex == cart[i].index ) {
            console.log('เจอสินค้าที่เหมือนกัน!')
            cart[i].count++;
            pass = false;
        }
    }
    ...
    Swal.fire({
        icon: 'success',
        title: 'เพิ่ม ' + product[productindex].name + ' ลงในรถเข็นแล้ว !'
    })
...
function rendercart() {
    if(cart.length > 0) {
        ...
        $("#mycart").html(`<p>ไม่พบสินค้าในรถเข็นของคุณ</p>`)
    }
}
    ...
    if(action == '-') {
        if(cart[index].count > 0) {
            ...
            if(cart[index].count <= 0) {
                Swal.fire({
                    icon: 'warning',
                    title: 'คุณแน่ใจว่าจะนำสินค้าออกจากรถเข็นใช่ไหม?',
        ...
        data: {
            ...
            if(response.RespCode == 200) {
                Swal.fire({
                    icon: 'success',
                    title: 'ขอบคุณค่ะ',
                    html: ` <p> จำนวนเงิน : ${response.Amount.Amount}</p>
                            <p> ค่าบริการ : ${response.Amount.Shipping}</p>
                            <p> ภาษีมูลค่าเพิ่ม 7% : ${response.Amount.Vat}</p>
                            <p> รวมเงิน : ${response.Amount.Netamount}</p>
                            `
                }).then((res) => {
                    if(res.isConfirmed) {
                        cart = [];
